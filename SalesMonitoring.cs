...
namespace BMC2021
{
    public partial class SalesMonitoring : Form {
        ...
        protected void cb_item_selectedIndexChanged(object sender, EventArgs e) {
            ...
            var query =
            from ord in db.Inv_Summary
            where ord.brand == selectedBrand && ord.itemname == selected
            select ord;

            foreach (Inv_Summary ord in query) {
                lb_price.Text = Convert.ToString(ord.saleprice);
            }
        ...
        private void btn_addtolist_new_Click(object sender, EventArgs e) {
            ...
            if (brands == "") {
                ...
                //SAVE TO DATATABLE
                DataRow _ravi = orders.NewRow();
                _ravi["itembrand"] = brands;
                _ravi["itemname"] = items;
                _ravi["qty"] = qty;
                _ravi["discount"] = discount;
                _ravi["price"] = prices;
                _ravi["totalprice"] = ttprice;
        ...
        private void RefreshGrid_customerlist() {
            if (grid_customerlist.Rows.Count == 0) { //do nothing
            } else {
                try {
                    int selectedrowindex = grid_customerlist.SelectedCells[0].RowIndex;
                    DataGridViewRow selectedRow = grid_customerlist.Rows[selectedrowindex];
            ...
            if (CheckBeforeOut() == true) {
                ...
                if (result == DialogResult.Yes) {
                    ...
                    foreach (DataRow row in orders.Rows) {
                        ...
                        string obrand = row.Field<string>("itembrand");
                        string oiname = row.Field<string>("itemname");
        ...
        private void printDeliveryReceipt() {
            ...
            try {
                ...
                printer.SubTitle = "DELIVERED TO: " + selectedCustomer + "   DATE: " + DateTime.Today.ToString();
        ...
        private void grid_customerlist_CellContentClick(object sender, DataGridViewCellEventArgs e) {
